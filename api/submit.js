...
const required = ['firstName','lastName','countryCode','mobile','deviceFingerprint','semiOne','semiTwo','result','champion','manOfMatch','goldenBoot'];
...
export default async function handler(req, res) {
  ...
  const mobileNumber = String(body.mobile).replace(/\D/g, '');
  const countryCode = String(body.countryCode).replace(/[^0-9+]/g, '');
  if (!/^[6-9]\d{9}$/.test(mobileNumber) || countryCode !== '+91') return res.status(400).json({ error: 'Please enter a valid Indian mobile number.' });
  ...
  const entry = { first_name: body.firstName.trim(), last_name: body.lastName.trim(), country_code: countryCode, mobile_number: mobileNumber, device_fingerprint: body.deviceFingerprint, team_1: body.semiOne, team_2: body.semiTwo, team_1_score: Number(body.scoreOne), team_2_score: Number(body.scoreTwo), final_result: body.result, winning_team: body.champion, penalty_winner: body.penaltyWinner || null, first_goalscorer: String(body.firstGoalscorer || '').trim() || null, man_of_match: body.manOfMatch.trim(), golden_boot: body.goldenBoot.trim() };
  try {
    const response = await fetch(`${process.env.SUPABASE_URL}/rest/v1/competition_entries`, { method:'POST', headers:{ apikey:process.env.SUPABASE_SECRET_KEY, Authorization:`Bearer ${process.env.SUPABASE_SECRET_KEY}`, 'Content-Type':'application/json', Prefer:'return=representation' }, body:JSON.stringify(entry) });
    const result = await response.json();
    if (!response.ok && (response.status === 409 || result?.code === '23505')) return res.status(409).json({ error: 'This mobile number or device has already submitted a prediction.' });
    if (!response.ok) { console.error(result); throw new Error('Database error'); }
