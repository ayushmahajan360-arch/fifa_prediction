...
const required = ['firstName','lastName','email','countryCode','mobile','semiOne','semiTwo','result','champion','manOfMatch','goldenBoot'];
...
export default async function handler(req, res) {
  ...
  if (mobileNumber.length < 7 || mobileNumber.length > 15) return res.status(400).json({ error: 'Please enter a valid mobile number.' });
  if (!/^\S+@\S+\.\S+$/.test(String(body.email))) return res.status(400).json({ error: 'Please enter a valid email address.' });
  ...
  const entry = { first_name: body.firstName.trim(), last_name: body.lastName.trim(), email: body.email.trim().toLowerCase(), country_code: countryCode, mobile_number: mobileNumber, team_1: body.semiOne, team_2: body.semiTwo, team_1_score: Number(body.scoreOne), team_2_score: Number(body.scoreTwo), final_result: body.result, winning_team: body.champion, penalty_winner: body.penaltyWinner || null, first_goalscorer: String(body.firstGoalscorer || '').trim() || null, man_of_match: body.manOfMatch.trim(), golden_boot: body.goldenBoot.trim() };
  try {
